"""回填 market_emotion 20260804(不含) -> 20260811(纯库内计算, 零网络请求)

动机: market_emotion 表生产实跑仅存到 20260804, 0805->0811 缺 5 个交易日。
本脚本只读本地规范库表(stock_kline/stock_info), 复用生产计算函数落库。
模式与 data/backfill_market_emotion_20260804.py 完全一致, 仅改回填区间。
...
字段口径(与生产 core/market_environment.py + main.py 对齐, 差异已注明):
  - limit_up_count / limit_down_count:
      ...
      (2026-07-16 口径切换点之后的新口径, 不复刻历史生产行 top50 截断。)
...
用法: cd src && python3 data/backfill_market_emotion_20260811.py
"""
...
START_EXCLUSIVE = "20260804"  # 回填区间: (0804, 0811]
END_DATE = "20260811"
