"""回补龙虎榜 20260715 -> 20260804(串行, 反爬安全)
...
数据源策略(canonical, 绕开被墙的 eastmoney):
  ...
  - 富化: pywencai 日期限定查询 "YYYY年M月D日龙虎榜" 补 净买额/买入额/卖出额
    * 校验: 返回列名必须带请求日期后缀(如 当日龙虎榜净额[20260715]),
...
落库: data/stock_history.db.dragon_tiger (WAL, busy_timeout=60000)
  列: stock_code, trade_date, net_buy, buy_amount, sell_amount, seats(NULL),
      has_hot_money(0), reason(本脚本 ALTER TABLE 增补)
  Fail-Loud: 缺 stock_code/name 的行由 DataNormalizer 丢弃并记日志, 不填中性值。

用法: cd src && python3 data/backfill_dragon_tiger_20260804.py
"""
...
def get_trade_dates(conn) -> list:
    """从规范库 stock_kline 取区间内的真实交易日(权威日历)。"""
    rows = conn.execute(
